#Feature engineering functions for Bike project
...
def fe_test(data2):
    ...
    pipeline1 = Pipeline([
        ('scale1', StandardScaler(), [ 'temp']),
        ('poly_features1', PolynomialFeatures(degree=8, include_bias=False))
    ])

    pipeline2 = Pipeline([
        ('scale2', StandardScaler(), [ 'windspeed']),
        ('poly_features2', PolynomialFeatures(degree=8, include_bias=False))
    ])

    c_transformer = ColumnTransformer([
    ('pass', 'passthrough', ['sin_time', 'cos_time']),
    ('scale1 and poly_features1', pipeline1(), [ 'temp']),
    ('scale2 and poly_features2', pipeline2(), [ 'windspeed']),
    ])
